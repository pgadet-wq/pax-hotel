...
- 19/09 (hors phase) — **ingestion de la liste passagers compagnie : spécifiée PUIS implémentée**.
  ...
  - Reste ouvert (§6 de la spéc) : écran de correspondance RBD → cabine (un export DCS qui ne porte que des lettres est refusé), `computeNeeds` compte encore les dossiers hors plan, purge des sorties nominatives après séance, livrable « rooming list par hôtel ».
