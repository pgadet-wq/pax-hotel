...
change réellement dans le plan, et ce que fait le code **aujourd'hui** quand la règle n'est pas tenue.

**État : spécifiée, pas encore implémentée.** Le lecteur CSV actuel (`hai-admin-mcp/lib/csv.mjs`) ne valide
aucune valeur ; le lot de mise en œuvre est décrit au §6. Fichiers à transmettre à la compagnie :
...
1. **Une valeur que l'outil ne comprend pas ne devient jamais une valeur par défaut.** Elle produit un refus
   nommé (ligne, colonne, valeur) ou un avertissement tracé dans le rapport d'ingestion. Aujourd'hui, une
   cabine `C` devient `Y`, un type `ADULT` ne compte personne, un `WCHC` ne déclenche pas PMR, un PNR vide
...
| **Guillemets** | RFC 4180 : tout champ contenant le séparateur, un guillemet ou un saut de ligne est encadré de `"`, guillemets internes doublés. *Aujourd'hui le parseur ne sait pas relire un champ cité : `"LE GOFF;MARIE"` décale toutes les colonnes suivantes et fabrique un « adulte-zéro » en cabine Y, sans erreur.* |
| **En-tête** | Obligatoire, première ligne. **Ordre des colonnes libre.** Noms en minuscules sans accent, mais comparaison insensible à la casse, aux accents, aux espaces, tirets et underscores, via table d'alias. Colonnes surnuméraires tolérées et listées comme « ignorées ». Deux en-têtes qui normalisent vers le même nom = refus (*aujourd'hui la dernière colonne écrase silencieusement la première*). |
...
| **assistance** | Codes SSR IATA, plusieurs acceptés (espace, virgule ou `/`). **À effet hôtel** : `WCHR`, `WCHS`, `WCHC`, `WCBD`, `WCBW`, `WCMP`, `BLND`, `DEAF`, `DPNA`, `EXST` → surcouche PMR. **Escalade nominative** : `UMNR`/`UNN`, `MEDA`, `STCR`. **Reconnus sans effet chambre** : `MAAS`, `BSCT`, `NSST`, `BULK`, `SPEQ`, repas `*ML`. `PETC`/`AVIH` → note animaux. Code inconnu : avertissement, jamais refus. | Surcouche **PMR** : file en tête de traitement, **chambre accessible exigée**, poids distance ×2, surclassement de tier autorisé, dérogation de plafond en dernier recours, note « transfert adapté à confirmer ». | « 0 PMR reconnu » en tête de rapport. *Aujourd'hui seul le code exactement `WCHR` compte : `WCHS`, `WCHC`, `BLND`, `DEAF`, `wchr` et même `WCHR BLND` ne déclenchent rien.* |
...
| **droit_entree** | `OUI`, `NON`, `INCONNU` (alias `TWOV`, `TRANSIT ONLY` → NON ; vide → INCONNU). | `NON` ou `INCONNU` : le dossier ne part pas à l'hôtel de ville, il sort en escalade **« droit d'entrée — traitement nominatif GHA »**. `INCONNU` n'est jamais rabattu sur `OUI`. | Avertissement : « tous les dossiers planifiés comme entrants ; une nuit à BKK dépasse le seuil de transit sans visa ». *La fiche escale BKK déclare `entry_visa_check: true` — aucune ligne de code ne le lit aujourd'hui.* |
...
| **email** / **telephone** | Adresse ou numéro, par personne ou répété sur le PNR. | Alimente une colonne `destinataire` du fichier messages. *Aujourd'hui les 300+ messages FR/EN produits ne sont adressables à personne : ils renvoient au comptoir.* Donnée sensible : jamais transmise à un tiers. |
| **age** | Entier 0–120, ou vide. | **Contrôle croisé du type** seulement (ADT < 12 ans, CHD ≥ 18, INF ≥ 2 → avertissement). *Aujourd'hui cette colonne est obligatoire à fournir et n'est lue par aucun module.* Elle devient facultative. |
...
2. **Repli** — le fichier ne porte que le RBD : à l'import, l'écran liste les lettres rencontrées
   (`C · D · S · M · K…`) et l'opérateur affecte J/W/Y à chacune en quelques clics. La correspondance est
   enregistrée pour les imports suivants. **Aucun refus massif de fichier pour une table manquante.**
...
## 6. Ce que la mise en œuvre implique (côté outil)

Charge mesurée : **≈ 2,75 jours**, sans toucher au lecteur CSV actuel (verrouillé par des tests de
